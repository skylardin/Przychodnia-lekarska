...
\begin{itemize} \item Data rozpoczêcia: 10.01.2018. \item Data zakoñczenia: 24.01.2018. \end{itemize}

\subsection{Sprint 4}

\begin{itemize} \item Data rozpoczêcia: 10.01.2018 \item Data zakoñczenia: 24.01.2018. \item Scrum Master: Pawe³ Wilczek. \item Product Owner: £ukasz Mamak \item Development Team: £ukasz Szkaradek, £ukasz Mamak, Pawe³ Wilczek. \end{itemize}
...
\paragraph{Tytu³ zadania.} Mo¿liwoœæ automatycznego wpisywania danych logowania pacjenta po IP. \begin{itemize} \item Estymata: L. \end{itemize}

\paragraph{Tytu³ zadania.} Poprawa formularza rejestracji pacjenta.  \begin{itemize} \item Estymata: S. \end{itemize}

\paragraph{Tytu³ zadania.} Modyfikacja procedury rejestracji pacjenta.  \begin{itemize} \item Estymata: L. \end{itemize}

\paragraph{Tytu³ zadania.} Dodanie opcji wydruku karty pacjenta.  \begin{itemize} \item Estymata: L. \end{itemize}
...
Zadanie umo¿liwia zmianê has³a dla u¿ytkowników recepcji, dyrektora, lekarza oraz recepcjonisty. Po zalogowaniu siê osoby o wymienionych uprawnieniach, pojawia siê przycisk przekierowuj¹cy do odpowiedniego formularza, który wymaga autoryzacjê bie¿¹cym has³em i umo¿liwia jego zmianê.
Kod programu (œrodowisko \texttt{verbatim}): \begin{verbatim} 
if(isset($_GET['haslo'])) {
if ($_GET['stan']=='h') { 
echo "B³¹d, has³a nie s¹ takie same!";
...
echo '<form action="recepcja.php?haslo" method="post"> Podaj stare has³o: <input type="password" name="starehaslo"><br>
Podaj nowe has³o: <input type="password" name="haslo1"><br>
Powtórz nowe has³o: <input type="password" name="haslo2"><br> 
<input type="submit" value="Zmieñ has³o"></form>';
}
...
\paragraph{Tytu³ zadania.}  Mo¿liwoœæ automatycznego wpisywania danych logowania pacjenta po IP. \subparagraph{Wykonawca.} Pawe³ Wilczek, £ukasz Mamak \subparagraph{Realizacja.} Funkcjonalnoœæ, która ma na celu u³atwienie korzystania ze strony internetowej przychodni. Dodana zosta³a nowa tablica, zawieraj¹ca w sobie informacje o IP,  numerze pesel oraz kod. Gdy zostanie zaznaczona opcja zapisu, przy nastêpnym logowaniu nasze dane zostan¹ sczytane i uzupe³nione w sposób automatyczny. Realizacja przebieg³a planowo. 
...
Kosmetyczna poprawa w kodzie Ÿród³owym formularza rejestracji pacjenta, zmiana pól pesel oraz numer telefonu na numeryczne.
Kod programu (œrodowisko \texttt{verbatim}): \begin{verbatim} 
<td>Pesel:</td><td><input type="number" name="pesel"></td><tr>
<td>Numer telefonu:</td><td><input type="number" name="tel"></td><tr>
\end{verbatim}

\paragraph{Tytu³ zadania.}  Modyfikacja procedury rejestracji pacjenta.  \subparagraph{Wykonawca.} £ukasz Mamak\subparagraph{Realizacja.} Zadanie polega³o na dokonaniu drobnej zmiany samej procedury rejestracji poprzez wzbogacenie jej o nowo dodan¹ funkcjê zwi¹zan¹ z auto uzupe³nianiem danych po adresie IP. 
Nie zosta³y napotkane ¿adne warte do odnotowania komplikacje.
\newline
\newline
Kod programu (œrodowisko \texttt{verbatim}): \begin{verbatim} 
...
echo "Wpisano do kolejki.<br>Proszê zapamiêtaæ swój kod: " . $randomString;
echo '<br><br><br><a style="color:red" href="index.php">Powrót do strony g³ównej.</a><br>'; 
}
?>
...
\paragraph{Tytu³ zadania.}  Dodanie opcji wydruku karty pacjenta.  \subparagraph{Wykonawca.} Pawe³ Wilczek\subparagraph{Realizacja.} Zadanie polega³o na napisaniu skryptu który pozwoli³by na drukowanie karty pacjenta. Rozwi¹zanie to  pozwala na druk i udostêpniane dokumentacji medycznej w formie papierowej, co jest zgodne z  przepisami ustawy z dnia 6 listopada 2008 r. o prawach pacjenta i Rzeczniku Praw Pacjenta.  Wykonanie zadania przebieg³o pomyœlnie, bez kompilacji b¹dŸ b³êdów.
\newline
\newline
Kod programu (œrodowisko \texttt{verbatim}): 
\begin{verbatim} 
<?php
...
\subsection{Sprint Review/Demo}\label{Sprint4demo} Sprint podsumowuj¹cy ca³¹ pracê przebieg³ zgodnie z oczekiwaniami. Naniesione zosta³y istotne poprawki, dodane zosta³y nowe funkcje. Wszystko zosta³o zrealizowane na czas.
\newline
\newline
\textit{Demonstracja przyrostu produktu:}\newline
\newline
